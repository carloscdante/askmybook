...
def construct_prompt(question: str, context_embeddings: dict, df: pd.DataFrame) -> tuple[str, str]:
    ...
    header = """Fireflies.AI is an AI notetaking software for meetings that optimizes work and automatically takes meeting notes. These are questions and answers regarding the platform. Please keep your answers to three sentences maximum, and speak in complete sentences. Stop speaking once your point is made. Answer only questions about Fireflies.AI and don't go off topic. If someone asks you something outside of the Fireflies.AI context, explicitly and politely say you can't answer the question.\n\nContext that may be useful, pulled from the Fireflies.AI Help Desk:\n"""

    # question_1 = "\n\n\nQ: What are the settings to automatically create tasks on Monday.com?\n\nA: To automatically create tasks on Monday.com, select the relevant Board from the drop-down menu to create tasks from your meetings. Select the Group where the item would be created. Select at least one Topic so that Fireflies can create items based on mentions of these keywords. Voice commands are the default Topic that will be selected by default. Based on the voice commands which you have created, items would be created within your Monday.com account under the chosen Board and Group."
